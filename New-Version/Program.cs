...
class Program
    {
        ...
        Storage storage = new Storage();
...

            {
                ...
                //Console.WriteLine("<<   Current spaces: empty >>");  Här är önskad funktion 1 där man kan förbärttra
                Console.WriteLine(" <<<<<<<<<<<<<<<<<<<<¤>>>>>>>>>>>>>>>>>>>>");
            ...
            switch (choice)
                ...
                case "1":
                    ...
                    Console.WriteLine("Please enter the lot number:");
                    int ticketLot = int.Parse(Console.ReadLine());
                    if (addInput == "CAR")
                    {
                        storage.AddCars(1, newPlateNum, ticketLot);
                    }
                    else if (addInput == "MC")
                    {
                        storage.AddMCs(1, newPlateNum, ticketLot);
                    }
                    else if (addInput == "EXIT")
                    {
                        return;
                    }
                    else
                ...
                case "3":
                    ...
                    int plotNum = 0;
                    foreach (CustomersVehicle vehicle in storage.pLot)
                    {
                        plotNum++;
                        if (vehicle == null)
                        {
...
class Storage
    {
        ...
        public CustomersVehicle[] pLot = new CustomersVehicle[100];
    ...
    public Storage()
        ...
        {
            pLot[i] = null; // Initialize each spot as empty
        }
    }

        // Method to add cars
        public void AddCars(int count, string plateNum, int ticketLot)
        {
            ...
            if (carCount + count <= Car.MaxCount && requiredSpace <= AvailableSpace)
            {
                ...
                pLot[ticketLot] = new CustomersVehicle(plateNum, "CAR", ticketLot);
    ...
    public void RemoveCars(int count, int ticketLot)
    {
        double requiredSpace = count * Car.SpacePerItem;
        if (carCount - count >= 0 && pLot[ticketLot] != null && pLot[ticketLot].VehicleType == "CAR")
        {
            carCount -= count;
            AvailableSpace += requiredSpace;
            pLot[ticketLot] = null;
    ...
    // Method to add MCs
    public void AddMCs(int count, string plateNum, int ticketLot)
        {
            ...
            if (mcCount + count <= MC.MaxCount && requiredSpace <= AvailableSpace)
            {
                mcCount += count;
                AvailableSpace -= requiredSpace;
            pLot[ticketLot] = new CustomersVehicle(plateNum, "MC", ticketLot);
    ...
    public void RemoveMCs(int count, int ticketLot)
        { 
            double requiredSpace = count * MC.SpacePerItem;
        if (mcCount - count >= 0 && pLot[ticketLot] != null && pLot[ticketLot].VehicleType == "MC")
        {
